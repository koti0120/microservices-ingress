pipeline {
    ...
    environment {
        DOCKER_HUB_REPO = 'koti0120/techsolutions-app'
        K8S_CLUSTER_NAME = 'koti-cluster'
    ...
    stages {
        stage('Checkout') {
            steps {
                ...
                git 'https://github.com/koti0120/microservices-ingress.git'
            }
        }
        ...
        stage('Deploy to Kubernetes') {
            steps {
                ...
                script {
                    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-creds']]) {
                        sh "sed -i 's|koti/techsolutions-app:latest|koti/techsolutions-app:${env.IMAGE_TAG}|g' k8s/deployment.yaml"
